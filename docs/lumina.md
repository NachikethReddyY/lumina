...
Lumina is a light-mode-only internal support operations platform. The base canvas is **warm cream** (`{colors.canvas}` — #f7f7f4) holding warm near-black ink (`{colors.ink}` — #26251e) for body and display alike. The single brand accent is **Lumina Green** (`{colors.primary}` — #166534) reserved for primary CTAs, kicker labels, and active UI states — used scarcely.

**The app is strictly light mode.** No dark theme. `color-scheme: only light` is set globally. Do not introduce dark-mode variants.

Type runs **Inter Variable** as the single sans family. Display sits at weight 400 with negative letter-spacing — an editorial voice. JetBrains Mono carries every code surface.
...
- Single accent color: `{colors.primary}` (Lumina Green #166534). Used scarcely.
...
### Brand & Accent
- **Lumina Green** (`{colors.primary}` — #166534): Primary CTAs, kicker badges, active nav states, accent labels. Used scarcely.
- **Lumina Green Hover** (`{colors.primary-hover}` — #15803d): Hover state.
- **Lumina Green Active** (`{colors.primary-active}` — #14532d): Press state. Also used for kicker badge text.
...
- **On Primary** (`{colors.on-primary}` — #ffffff): White text on Lumina Green.
...
### Font Family
**Inter Variable** is the display + body family. Fallback: `system-ui, "Helvetica Neue", Helvetica, Arial, sans-serif`. Code surfaces switch to **JetBrains Mono**.
...
Inter Variable is the active typeface. No substitution needed.
...
### Decorative Depth
- **IDE-mockup cards** are the only "elevated" element. White card on cream canvas with internal pane structure mimicking the actual Lumina app.
...
**`top-nav`** — Background `{colors.canvas}`, text `{colors.ink}`, height 64px. Layout: Lumina wordmark left, Sign In + "Try Lumina" secondary-dark CTA right.

### Buttons

**`button-primary`** — The primary Lumina Green CTA. Background `{colors.primary}`, text `{colors.on-primary}`, type `{typography.button}` (14px / 500), padding 10px × 18px, height 40px, rounded `{rounded.md}` (8px).
...
**`comparison-card`** — Side-by-side comparison card. Same surface and rounding; internally split into 2 columns.
...
**`cta-band`** — Pre-footer CTA card. Centered white card on canvas, display headline in `{typography.display-lg}`, single secondary-dark CTA. 80px vertical padding.
...
### Do
- Reserve `{colors.primary}` (Lumina Green) for primary CTAs, kicker badges, and active nav states.
...
### Don't
- Don't introduce a secondary brand action color. Lumina Green is the only accent.
...
6. Inter Variable 400 for display, 400/500/600 for body. JetBrains Mono on every code surface.
7. Lumina Green stays scarce.
8. Timeline pastels stay scoped to in-product agent visualizations.

## Known Gaps

- Inter Variable is the active typeface. Light mode only — no dark theme support.
