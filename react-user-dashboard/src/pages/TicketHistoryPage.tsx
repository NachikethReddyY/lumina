...
function StatusIcon({ status }: { status: string }) {
  if (status === 'resolved' || status === 'closed') return <CheckCircle2 size={12} />;
  if (status === 'in_progress') return <Clock size={12} />;
  if (status === 'open' || status === 'pending_routing') return <AlertCircle size={12} />;
...
function TicketCard({ ticket, onClick }: { ticket: ApiTicket; onClick: () => void }) {
  ...
  return (
    <motion.div
      ...
      style={{ borderTopColor: PRIORITY_COLOR[p] }}
    >
      <div className="th-card-top">
        <span className="th-priority-badge" style={{ color: PRIORITY_COLOR[p], background: `${PRIORITY_COLOR[p]}18` }}>
          {p}
        </span>
        <span className="th-status-chip" style={{ color: STATUS_COLOR[s], background: `${STATUS_COLOR[s]}18` }}>
          <StatusIcon status={s} />
          {s.replace(/_/g, ' ')}
        </span>
      </div>
      <h3 className="th-card-title">{ticket.title}</h3>
      <p className="th-card-desc">{ticket.description}</p>
      <div className="th-card-footer">
        <span className="th-card-meta">{ticket.category_name}</span>
        <span className="th-card-date">{new Date(ticket.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span>
      </div>
      {ticket.assigned_to_name && (
        <div className="th-card-assignee">→ {ticket.assigned_to_name}</div>
      )}
    </motion.div>
  );
}
