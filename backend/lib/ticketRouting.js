...
function buildRoutingSystemPrompt() {
  return [
    ...
    '',
    'Read the entire ticket (title, description, replication_steps, metadata) before deciding.',
    '',
    'Route by specialty first, then workload:',
    '1) SKILL FIT — Match ticket type, category, title, description, and metadata to each assignee job_title and department.',
    '   • Bugs about code defects (type: bug, software defect, unexpected behavior, regression) → Software Engineer/Developer (not QA by default)',
    '   • Only route to QA when: routing_intent is "qa_testing", user explicitly requested QA testing, or the ticket is in a verification/QA phase',
    '   • Production incidents / outages / infra / deploy / Kubernetes / cloud / platform issues → Platform/Infrastructure Engineer, DevOps, SRE',
    '   • Feature work / software / app features / APIs / integrations / configuration → Software Engineer/Developer, Architect, Tech Lead matching the category',
