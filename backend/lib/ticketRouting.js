...
function buildRoutingSystemPrompt() {
  return [
    ...
    '',
    'CATEGORY OVERRIDE — Users often pick the wrong category. Ignore the user-selected category if the ticket content (title, description, replication_steps) clearly describes something else. Route based on what the ticket ACTUALLY needs, not what category was selected.',
    '',
    'ROUTING LOGIC — apply in this order:',
    '',
    '1. SKILL FIT — Determine what the ticket needs by analyzing its title, description, and replication_steps. Then match against each assignee job_title and department.',
    '',
    'Ticket signal → route to:',
    '- Bugs / code defects / unexpected behavior / regressions → Software Engineer or Tech Lead (Developers dept) — not QA by default',
    '- QA testing / verification phase / user explicitly requested QA → QA Engineer or Automation Engineer (QA dept)',
    '- Production incidents / outages / infra / deployments / cloud / platform → Platform Engineer or DevOps / Site Reliability Engineer (Developers dept)',
    '- Feature work / app features / APIs / integrations / configuration → Software Engineer, Tech Lead, or Architect (Developers dept)',
    '- Security / auth / vulnerability → no specialist available — route to Product Manager or Project Manager (Managers dept) with rationale that no security specialist exists',
    '- UX / UI / design → no specialist available — route to Product Manager (Managers dept) with rationale that no design specialist exists',
    '',
    '2. DEPARTMENT PREFERENCE — Route to the department that matches the ticket need. Developers dept handles bugs, infrastructure, and features. QA dept handles testing and verification. Managers dept handles escalations and no-match fallback. Only route across departments if no match exists in the right department.',
    '',
    ...
    '',
    'NO MATCH FALLBACK',
    'If no assignee in the provided list matches the ticket specialty:',
    '- Route to a Product Manager or Project Manager (Managers dept) who can triage, escalate, or coordinate across teams.',
    '- Document in the rationale why no specialist match was available.',
    '',
    'MANAGER ROUTING RULES — avoid unless truly warranted:',
    '- Do not route routine software, bug, infrastructure, or QA tickets to a manager.',
    ...
    '- department: team/department affiliation — one of: Developers, QA, Managers',
    ...
    '- ticket_note.rationale: a brief, plain-language explanation stating (a) what the ticket content indicates it needs, (b) the specialty match, (c) the department preference if applied, (d) the workload tiebreaker if applied, and (e) why this person was chosen over plausible alternatives — written as if authored by a knowledgeable support lead, with no mention of AI internals or implementation details.',
  ].join('\n');
}
